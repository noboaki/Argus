...
> Go로 구현한 경량 인프라 모니터링 시스템

Prometheus에서 영감을 받아, Agent가 각 서버의 시스템 메트릭을 수집하고 중앙 서버(Argus Server)로 실시간 전송하는 구조로 설계되었습니다.
...
| 보안 | TLS (선택적 적용) | 인증서 기반 암호화, 환경변수로 활성화 |
...
Argus Server와 Agent 간의 gRPC 통신에 TLS를 선택적으로 적용할 수 있습니다. 기본값은 평문(insecure) 통신이며, 인증서 경로를 환경변수로 지정하면 자동으로 TLS가 활성화됩니다.

### TLS 비활성화 (기본값)

```
ARGUS_TLS_ENABLED=false  → 평문 통신 (개발/로컬 환경)
```

### TLS 활성화

```
ARGUS_TLS_ENABLED=true   → TLS 암호화 통신 (운영 환경 권장)
```

**서버 측 (Argus Server)**

```bash
ARGUS_TLS_ENABLED=true \
ARGUS_TLS_CERT_FILE=/certs/server.crt \
ARGUS_TLS_KEY_FILE=/certs/server.key \
...
```bash
ARGUS_TLS_ENABLED=true \
ARGUS_TLS_CA_FILE=/certs/ca.crt \
docker compose up --build
```

> mTLS(상호 인증)가 필요한 경우 Agent에도 `ARGUS_TLS_CERT_FILE`, `ARGUS_TLS_KEY_FILE`을 추가로 설정합니다.
...
openssl req -new -x509 -days 365 -key ca.key -out ca.crt -subj "/CN=ArgusCA"

# 서버 키 및 CSR 생성
openssl genrsa -out server.key 4096
openssl req -new -key server.key -out server.csr -subj "/CN=argus-server"
...
| `ARGUS_TLS_ENABLED` | `false` | TLS 활성화 여부 |
| `ARGUS_TLS_CA_FILE` | - | CA 인증서 경로 (TLS 활성화 시) |
| `ARGUS_TLS_CERT_FILE` | - | 클라이언트 인증서 경로 (mTLS 사용 시) |
| `ARGUS_TLS_KEY_FILE` | - | 클라이언트 키 경로 (mTLS 사용 시) |
...
| `ARGUS_TLS_ENABLED` | `false` | TLS 활성화 여부 |
| `ARGUS_TLS_CERT_FILE` | - | 서버 인증서 경로 (TLS 활성화 시) |
| `ARGUS_TLS_KEY_FILE` | - | 서버 키 경로 (TLS 활성화 시) |
| `ARGUS_TLS_CA_FILE` | - | CA 인증서 경로 (mTLS 사용 시) |
...
`ARGUS_TLS_ENABLED` 환경변수 하나로 평문/TLS 모드를 전환합니다. 개발 환경에서는 인증서 없이 빠르게 실행하고, 운영 환경에서는 동일한 바이너리에 인증서 경로만 추가해 TLS를 활성화할 수 있습니다. mTLS(상호 인증)도 지원하므로 Agent가 서버를 검증하는 단방향 TLS와, 서버가 Agent도 검증하는 양방향 TLS 모두 구성 가능합니다.
